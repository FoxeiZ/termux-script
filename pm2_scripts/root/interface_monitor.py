...
if TYPE_CHECKING:
    ...
    from lib.types import Embed, EmbedField


class InterfaceMonitorConfigData(IntervalConfigT):
    REBOOT: bool
...
class InterfaceMonitorConfig(IntervalConfigLoader[InterfaceMonitorConfigData]):
    ...
    def on_add_arguments(self, parser: argparse.ArgumentParser) -> None:
        parser.add_argument(
            ...
            action="store_true",
            dest="REBOOT",
            help="Enable automatic reboot after prolonged network loss",
        )
        parser.add_argument(
            "--reboot-threshold",
            type=int,
            default=1800,
            dest="REBOOT_THRESHOLD",
            help="Time in seconds to wait before rebooting after network loss (default: 1800)",
        )
        parser.add_argument(
            "--hotspot",
            action="store_true",
            dest="HOTSPOT",
            help="Enable automatic WiFi hotspot when network is lost",
        )
...
class InterfaceMonitorPlugin(IntervalPlugin):
    ...
    def __init__(self, config: InterfaceMonitorConfig) -> None:
        super().__init__(config)

        self.username = "RN10P"
        self.avatar_url = "https://cdn.discordapp.com/app-assets/1049685078508314696/1249009769075703888.png"

        self.reboot_enabled = config.reboot
        self.reboot_threshold = config.reboot_threshold
        self.hotspot_enabled = config.hotspot
...
if __name__ == "__main__":
    ...
    plugin = InterfaceMonitorPlugin(config)
