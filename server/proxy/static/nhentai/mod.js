  ...
  fetch(`/func/add/${galleryId}`, {
    ...
    .then((data) => {
      if (
        ...
        data.message === "Gallery is already being downloaded"
      ) {
...
function showCancelButton(galleryId) {
  ...
  if (!cancelButton) {
    ...
    cancelButton.style.cssText =
      "min-width: unset; padding: 0 0.75rem; margin-left: 0.5rem;";
    cancelButton.href = "#";
...
document.addEventListener("DOMContentLoaded", function () {
  const galleryId = parseInt(
    document.getElementById("gallery_id").innerText.replace("#", "")
  );
  fetch(`/func/progress/${galleryId}`)
    .then((response) => response.json())
    .then((data) => {
      if (
        !data.error &&
        (data.status === "pending" || data.status === "downloading")
      ) {
        const addButton = document.getElementById("add");
        addButton.classList.add("btn-disabled");
        showCancelButton(galleryId);
